...
BOOKING_FLOW_FILE = Path("data/booking_flows.json")
BOOKING_FLOW_TIMEOUT_MINUTES = 30
OFFER_CACHE_TIMEOUT_MINUTES = 60
HOTEL_BOOKINGS_DB = Path("data/hotel_bookings.db")
...
def _load_booking_flows() -> Dict[str, Any]:
    if not BOOKING_FLOW_FILE.exists():
        return {}
    try:
        with open(BOOKING_FLOW_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}
...
def _save_booking_flows(data: Dict[str, Any]) -> None:
    BOOKING_FLOW_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(BOOKING_FLOW_FILE, "w", encoding="utf-8") as f:
